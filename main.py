...
class Like(db.Model):
    ...
    def by_username_by_blog_id(cls, username, blog_id):
        l = Like.all().filter('username =', username.lower()).filter(
            'blog_id =', blog_id).get()
        return l
# End Like Class
...
class LikeBlogEntryPage(Handler):
    ...
    def get(self, blog_id):
        if self.user:
            ...
            existing_like = Like.by_username_by_blog_id(
                self.user.username, int(blog_id))

            if not existing_like and (blog.username != self.user.username):
                new_like = Like(
                    blog_id=int(blog_id), username=self.user.username)
                new_like.put()
